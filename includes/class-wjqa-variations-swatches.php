<?php
/**
 * Feature B, swatches path — reconnect Variation Swatches Pro's archive swatches.
 *
 ...
 *
 * Selected by WJQA_Archive_Variations when the site has the Pro archive component.
 *
 ...
 * `handleVariationSwatchesAddToCart` (assets/js/frontend.js) listens for it, finds
 * `.wvs-add-to-cart-button` in the card, and rewrites it into a native WooCommerce
 * AJAX add-to-cart pointing at the chosen variation ID.
 *
 * WHAT IS MISSING, AND WHY
 *
 * 1. Swatches are never printed. Variation Swatches Pro hooks
 *    `woocommerce_after_shop_loop_item`; JetWooBuilder renders its own card and never
 *    fires it.
 *
 * 2. The button never gets the `wvs-add-to-cart-button` class the integration looks
 *    for. Variation Swatches Pro adds it through `woocommerce_loop_add_to_cart_args`,
 *    but JetWooBuilder assembles its own args and calls
 *    `wc_get_template( 'loop/add-to-cart.php' )` directly instead of
 ...
 *    JetWooBuilder cards, so the button is never rewired and a click just follows its
 *    href to the product page. Handled in assets/js/shop-cards.js by calling the
 *    plugin's own public jQuery plugin.
 *
 * Points 1 to 3 are fixed here, through documented extension points — no template
 * overrides are needed.
 */
class WJQA_Variations_Swatches {
	 ...
	 * `wvs_ajax_add_to_cart` marks the button as not yet resolved to a variation, and
	 * JetWooBuilder swaps it for `ajax_add_to_cart` once one is chosen.
	 *
	 ...
	 * the markup wholesale, so anything added earlier is discarded.
	 *
	 * The quantity and the button are wrapped together so they lay out as one row
	 * beneath the swatches, mirroring the `form.cart` JetWooBuilder builds for simple
	 * products.
	 *
	 ...
	 *
	 * On by default here, unlike the dropdown path. Swatches are a wide control that
	 * already occupies its own row, so the button sits on a second row with room
	 * beside it, and a site that has paid for archive swatches has usually designed
	 * the card around them.
	 *
	 * The meaningful bounds belong to the chosen variation, which is only known in the
	 * browser. The script narrows them on `show_variation`, and WooCommerce validates
